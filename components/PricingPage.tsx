...
const PricingPage: React.FC<PricingPageProps> = ({ t, lang }) => {
    ...
    <>
      {showContactModal && (
        <div className="fixed inset-0 z-[12000] flex items-center justify-center p-6 bg-slate-950/70 backdrop-blur-md animate-in fade-in duration-500">
          <div className="max-w-md w-full p-8 md:p-12 rounded-[40px] border-2 shadow-2xl relative text-center bg-white border-slate-200">
            <button onClick={handleCloseModal} className="absolute top-6 right-6 w-10 h-10 rounded-full flex items-center justify-center bg-slate-100 hover:bg-rose-500 hover:text-white transition-all text-slate-900 font-black">✕</button>
            
            <div className="w-20 h-20 mx-auto mb-8 rounded-3xl flex items-center justify-center text-3xl shadow-xl bg-blue-50 text-blue-600 border border-blue-100">
                <UserIcon className="w-10 h-10" />
            </div>

            <h2 className="text-2xl font-black uppercase tracking-tight text-slate-950 mb-4">{t.contactModalTitle}</h2>
            <p className="text-sm font-bold text-slate-700 mb-8">{t.contactModalBody}</p>
            
            <div className="space-y-6">
                <div className="p-6 rounded-3xl border-2 bg-slate-50 border-slate-200">
                    <div className="text-lg font-black text-slate-950 text-center mb-6">{t.contactModalName}</div>
                    <div className="space-y-4">
                        <div className="flex items-center justify-between gap-4">
                            <span className="text-sm font-bold text-blue-600 tracking-wider">{t.contactModalPhone}</span>
                            <button 
                                onClick={handleCopy}
                                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 border-2 shrink-0 ${
                                    ...
                                    : 'bg-slate-200 border-slate-300 text-slate-700 hover:border-blue-700'
                                }`}
                            >
                        ...
                        <div className="flex items-center justify-between gap-4">
                            <span className="text-sm font-bold text-blue-600 tracking-wider break-all text-left">{t.contactModalEmail}</span>
                            <button 
                ...
                <button onClick={handleCloseModal} className="w-full bg-slate-200 py-4 rounded-2xl font-black uppercase text-xs tracking-widest text-slate-700 hover:opacity-80 transition-opacity">
                    {t.contactModalClose}
                </button>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="text-center space-y-4">
          <h2 className="text-5xl font-black uppercase tracking-tight text-slate-950">{t.pricingTitle}</h2>
          <p className="text-lg font-semibold text-slate-800">{t.pricingSubtitle}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          ...
          <div className="relative p-10 rounded-[48px] border-2 border-emerald-500/30 flex flex-col transition-all hover:scale-[1.02] shadow-xl bg-white border-slate-200">
            ...
            <p className="text-sm font-semibold text-slate-800 mb-8 flex-grow">{t.planFreeDesc}</p>
            <div className="mb-8 flex flex-col">
              <div>
                <span className="text-4xl font-black">{isEn ? '0 EUR' : '0 Ft'}</span>
                <span className="text-slate-700 font-semibold ml-2">/ {t.planFreeTime}</span>
              </div>
          ...
          <div className="relative p-10 rounded-[48px] border-2 border-blue-600/40 flex flex-col transition-all hover:scale-[1.05] shadow-2xl bg-white">
            ...
            <p className="text-sm font-semibold text-slate-800 mb-8 flex-grow">{t.planWeeklyDesc}</p>
            <div className="mb-8">
              <span className="text-4xl font-black">{isEn ? '0 EUR' : '0 Ft'}</span>
              <span className="text-slate-700 font-semibold ml-2">/ {t.planWeeklyTime}</span>
              <div className="text-rose-600 font-black text-xs uppercase mt-2 tracking-widest">{isEn ? 'DEMO VERSION' : 'DEMO VERZIÓ'}</div>
            </div>
            <ul className="space-y-4 mb-10">
              {t.pricingFeats.map((feat: string, i: number) => (
          ...
          <div className="relative p-10 rounded-[48px] border-2 border-violet-600/40 flex flex-col transition-all hover:scale-[1.05] shadow-2xl bg-white">
            ...
            <p className="text-sm font-semibold text-slate-800 mb-8 flex-grow">{t.planMonthlyDesc}</p>
            <div className="mb-8">
              <span className="text-4xl font-black">{isEn ? '0 EUR' : '0 Ft'}</span>
              <span className="text-slate-700 font-semibold ml-2">/ {t.planMonthlyTime}</span>
              <div className="text-rose-600 font-black text-xs uppercase mt-2 tracking-widest">{isEn ? 'DEMO VERSION' : 'DEMO VERZIÓ'}</div>
            </div>
            <ul className="space-y-4 mb-10">
              {t.pricingFeats.map((feat: string, i: number) => (
            ...
            <button className="w-full bg-slate-900 text-white py-5 rounded-3xl font-black text-xs uppercase tracking-widest transition-all hover:opacity-90 active:scale-95">
              {t.planMonthlyBtn}
            </button>
          </div>
        </div>
