...
const CompetitorCard: React.FC<{
  ...
  darkMode: boolean;
  t: any;
}> = ({ name, focus, diff, darkMode, t }) => {
  return (
    <div className={`p-8 rounded-[32px] border-2 shadow-sm transition-all duration-500 hover:scale-[1.02] hover:shadow-xl flex flex-col h-full ${
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <span className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-widest mt-1 block">Competitor</span>
        </div>
      </div>
      ...
      <div className={`mt-auto p-6 rounded-2xl border-2 transition-all ${
        darkMode ? 'bg-indigo-950/30 border-indigo-900/40 text-indigo-100' : 'bg-indigo-50 border-indigo-100 text-indigo-950'
      }`}>
...
const CompetitorSection: React.FC<CompetitorSectionProps> = ({ analysis, t, darkMode }) => {
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Market Trend Banner - More Integrated */}
      <div className={`p-10 rounded-[40px] border-2 shadow-sm transition-all flex flex-col md:flex-row items-center gap-8 ${
        darkMode ? 'bg-slate-950/40 border-indigo-900/20' : 'bg-white border-indigo-100'
      }`}>
